...
const EOrderDetails = ({orders}) => {
   
    let formated = []
    orders.map((val,ind)=>{
        formated = formated.concat(val?.data?.orderDetails)
    })
     const formatedData = formated.filter((val)=>val!=undefined).map((values,index)=>({id:index,value:values?.quantity,label:values.product}))
